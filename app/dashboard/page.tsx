...
const SatelliteMap = dynamic(() => import("../components/SatelliteMap"), {
  ...
  loading: () => (
    <div
      className="w-full h-96 flex items-center justify-center text-xs tracking-widest animate-pulse"
      style={{
...
export interface DarkPoolDeal {
  id: string;
  type: "PRE_LAUNCH" | "DISTRESSED" | "AUCTION" | string;
  location: string;
  propertyType: string;
  discount: number;
  source: string;
...
export interface TelemetryMetrics {
  systemScoreOverride: string | number;
  variance: string | number;
  projectedYield: string | number;
  verdict: string;
}
...
export type NavTabId = "description" | "lead" | "contract" | "history" | "darkpool" | "portfolio";
...
const NAV_ITEMS: Array<{ id: NavTabId; label: string; icon: string }> = [
...
function DataOriginBadge({ origin }: { origin: DataOrigin }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md border ${cfg.cls}`}
    ...
    >
      ...
      {origin.isDemoMode && (
        <span style={{ color: "#9B59FF", fontWeight: 700 }}>[DEMO]</span>
      )}
    </div>
  );
}
...
function StampOverlay({ verdict, visible }: { verdict: string; visible: boolean }) {
  ...
  return (
    <div className="ea-stamp-overlay">
      <div className="ea-stamp" style={{ color: s.color, borderColor: s.color }}>
        {s.text}
      </div>
    </div>
  );
  ...
  return (
    <div className="ea-metric">
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
        ...
        {suffix && !isNA && (
          <span style={{ fontFamily: "var(--mono)", fontSize: 9, color, opacity: 0.4 }}>{suffix}</span>
        )}
      </div>
    </div>
  );
    ...
    <div style={{ padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 20, fontWeight: 700, color: getScoreColor(cur) }}>
            {cur.toFixed(1)}
          </div>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 20, fontWeight: 700, color: getScoreColor(fut) }}>
            {fut.toFixed(1)}
          </div>
      ...
      <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontFamily: "var(--mono)", fontSize: 8.5, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: vCol }}>
          {verdict}
        </span>
        {decay > 4 && (
          <span style={{ fontFamily: "var(--mono)", fontSize: 8, color: "#FF4D2E", animation: "dot-pulse 1.5s infinite" }}>
            ⚠ CRITICAL DECAY
          </span>
        )}
      </div>
...
export default function Dashboard() {
  ...
  // ── 1. TOP-LEVEL AUTH STATE ─────────────────────────────────────────────────
  // Must be declared before any other hooks so hook count is always stable.
  const [authState, setAuthState] = useState<AuthState>({ status: "loading", user: null });

  // ── 2. ALL OTHER STATE — declared unconditionally before any early returns ──
  const scrollRef = useRef<HTMLDivElement>(null);
  const mounted = useClientOnly();

  const [activeTab, setActiveTab]               = useState<NavTabId>("description");
  ...
  // ── 3. AUTH EFFECT — single source of truth ─────────────────────────────────
  useEffect(() => {
    let isMounted = true;

    const verifyAuth = async () => {
      const { status: authStatus, user, error } = await getAuthStatus(3, 800);
      ...
      if (authStatus === "authenticated" && user) {
        ...
        setTimeout(async () => {
          ...
          if (retry.status === "authenticated" && retry.user) {
            ...
            setUserEmail(retry.user.email ?? null);
  ...
  const addLog = useCallback((msg: string) => {
    ...
    setSystemLogs((prev: string[]) => [`[${stamp}] ${msg}`, ...prev].slice(0, 20));
  }, []);
  ...
  const fetchDarkPool = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = (await res.json()) as DarkPoolResponse;
        setDarkPoolDeals(d.deals || []);
        addLog(`DARK_POOL_SCAN: ${d.matchesFound} opportunities`);
      }
    } catch {
  ...
  const generatePDF = useCallback(
    (content: string) => {
      ...
      doc.setFillColor(3, 5, 14);
      doc.rect(0, 0, 210, 297, "F");
      doc.setTextColor(214, 240, 228);
      ...
      doc.text(`REF: ${id}`, 20, 46);
      doc.text(`OPERATOR: ${userId?.slice(0, 12) ?? "ANON"}`, 20, 52);
      doc.setTextColor(214, 240, 228);
  ...
  const handlePortfolioUpload = async () => {
    ...
    try {
      const assets = JSON.parse(portfolioJson) as unknown[];
      ...
      if (res.ok) {
        const d = (await res.json()) as PortfolioAnalysis;
        setPortfolioAnalysis(d);
        addLog("PORTFOLIO_ANALYSIS_COMPLETE");
      }
    } catch {
  ...
  // All state setters (setLoading, setMessages, etc.) are captured via closure
  // from the declarations above — no redeclaration possible.
  const handleAction = async (
    ...
    setMessages((prev: ChatMessage[]) => [
      ...prev,
      { role: "user", content: input || `Audit: ${property} | ${location} | ${price}` },
    ]);
    setChatInput("");
    setLoading(true);
    ...
    const iv = setInterval(() => {
      if (step < STEPS.length) {
        setStatus(`[${step + 1}/5] ${STEPS[step]}`);
        addLog(`PROTOCOL_${STEPS[step]}`);
        step++;
      }
    }, 1500);
    ...
    try {
      ...
      const rd = (await res.json()) as AuditResponse;
      clearInterval(iv);

      if (res.ok) {
        setMessages((prev: ChatMessage[]) => [...prev, { role: "assistant", content: rd.data }]);
        setStatus("ANALYSIS_VERIFIED");
        addLog("FORENSIC_AUDIT_COMPLETE");

        const m = rd.telemetryMetrics ?? ({} as TelemetryMetrics);
        const safeScore: number | string =
          m.systemScoreOverride === "N/A" ? "N/A" : parseFloat(String(m.systemScoreOverride)) || 0;
        const safeVar: number | string =
          m.variance === "N/A" ? "N/A" : parseFloat(String(m.variance)) || 0;

        setScoreState(safeScore);
        setVarianceState(safeVar);
        setYieldState(parseFloat(String(m.projectedYield)) || 0);
        setVerdictState(m.verdict || "HOLD");
        setLastStamp(m.verdict || "HOLD");
        setDataOrigin(rd.dataOrigin ?? null);
        setHasRealData(rd.hasRealData ?? false);

        // Future audit
        try {
          const fr = await fetch("/api/future", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ location, propertyType: property, price, beds, baths, holdPeriodYears: 20 }),
          });
          if (fr.ok) {
            const fd = (await fr.json()) as FutureResponse;
            setFutureScoreState(fd.futureScore);
            setFutureVerdictState(fd.futureVerdict);
            setFutureEvents(fd.criticalEvents ?? []);
            addLog(`FUTURE_AUDIT: ${fd.futureVerdict}`);
            if (fd.futureScore < 4) setIsFaultActive(true);
          }
        } catch {
        ...
        if (
          (safeScore !== "N/A" && (safeScore as number) <= 3) ||
          m.verdict === "REJECT"
        ) {
          setIsFaultActive(true);
          addLog("CRITICAL_RISK_DETECTED");
        }

        await fetchData();
        fetchDarkPool().catch(console.error);
      } else {
        throw new Error(rd.error ?? "SERVER_ERROR");
      }
    } catch (err: unknown) {
      clearInterval(iv);
      setStatus("UPLINK_CRITICAL_FAILURE");
      addLog(`ERROR: ${err instanceof Error ? err.message : "UNKNOWN_ERROR"}`);
    } finally {
      setLoading(false);
    }
  };

  const formatVariance = (v: number | string): string => {
  ...
  // ── 8. DERIVED VALUES ───────────────────────────────────────────────────────
  const evidencePts = hasRealData
    ? (dataOrigin?.sourcesCount ?? 0) * 12 +
      Math.round(((scoreState !== "N/A" ? scoreState : 0) as number) * 2)
    : 0;

  // ── 9. CONDITIONAL RENDERS — after ALL hooks ─────────────────────────────────
  ...
  if (!mounted) {
    return (
      <div style={{ display: "flex", height: "100vh", background: "#03050E", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "var(--mono)", color: "var(--em)", fontSize: 11, letterSpacing: ".25em", animation: "dot-pulse 1.5s infinite" }}>
          ...
          <nav style={{ flex: 1, padding: "12px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
            {NAV_ITEMS.map((t) => (
              <button
                ...
                className={`ea-nav-item${activeTab === t.id ? " active" : ""}`}
              >
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative", zIndex: 10 }}>
          ...
          <header style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontFamily: "var(--mono)", fontSize: 9, opacity: 0.65 }}>{userEmail ?? "PENDING..."}</div>
              </div>
              <div style={{
          ...
          {futureEvents.length > 0 && (
            <div style={{ flexShrink: 0, padding: "10px 20px", borderBottom: "1px solid rgba(0,255,163,0.09)", background: "rgba(3,5,14,0.7)" }}>
              <div className="ea-label" style={{ fontSize: 7.5, color: "rgba(255,77,46,0.7)", marginBottom: 8 }}>
                Critical Events Timeline
              </div>
              <div style={{ display: "flex", gap: 8, overflowX: "auto" }} className="ea-scroll-none">
                {futureEvents.map((e: FutureEvent, i: number) => (
                  <div key={i} className="ea-glass" style={{ flexShrink: 0, padding: "8px 12px", minWidth: 110 }}>
                    ...
                    <div style={{ fontSize: 8, opacity: 0.55, marginTop: 2, maxWidth: 100, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "var(--mono)" }}>
                      {e.event}
                    </div>
                    <div style={{
                      fontSize: 7.5, fontWeight: 700, marginTop: 4, fontFamily: "var(--mono)",
                      color: e.severity === "CRITICAL" ? "#FF4D2E" : e.severity === "HIGH" ? "#FF6B00" : "#FFB800",
                    }}>
                      {e.severity}
                    </div>
                  </div>
          ...
          <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            ...
            {/* Left Panel — Evidence Board only */}
            {activeTab === "description" && (
              <div style={{
                ...
                <div className="ea-scroll-none" style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
                  <EvidenceCard title="Asset Parameters" stamp={lastStamp}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {([
                        ...
                        <div key={i}>
                          <label className="ea-label" style={{ display: "block", marginBottom: 5, fontSize: 7.5 }}>
                            {inp.l}
                          </label>
                          <input
                            className="ea-input"
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        <div>
                          <label className="ea-label" style={{ display: "block", marginBottom: 5, fontSize: 7.5 }}>Beds</label>
                          <input
                            type="number"
                            className="ea-input"
                            value={beds}
                        ...
                        <div>
                          <label className="ea-label" style={{ display: "block", marginBottom: 5, fontSize: 7.5 }}>Baths</label>
                          <input
                            type="number"
                            className="ea-input"
                            value={baths}
                  ...
                  {dataOrigin && (
                    <div className="ea-glass" style={{ padding: "10px 12px" }}>
                      ...
                      {dataOrigin.fallbackReason && (
                        <div style={{ fontFamily: "var(--mono)", fontSize: 8, opacity: 0.3, marginTop: 6 }}>
                          {dataOrigin.fallbackReason}
                        </div>
                      )}
                    </div>
                  ...
                  {verdictState === "PRIME_ASSET" && (
                    <div className="ea-glass" style={{ padding: "12px", textAlign: "center", borderColor: "rgba(255,184,0,0.3)", background: "rgba(255,184,0,0.05)" }}>
                      ...
                      <button
                        className="ea-btn-primary"
                        style={{ background: "linear-gradient(135deg,#CC9400,#FFB800)", width: "100%", fontSize: 9 }}
                ...
                <div style={{ padding: "12px 14px", borderTop: "1px solid rgba(0,255,163,0.08)" }}>
                  ...
                  <p style={{ fontFamily: "var(--serif)", fontSize: 10, opacity: 0.35, lineHeight: 1.5, marginBottom: 8 }}>
                  ...
                  <div style={{ maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", gap: 18 }}>
                    {dataOrigin && messages.length > 1 && (
                      <div className="ea-fadeup" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 14, borderBottom: "1px solid rgba(0,255,163,0.08)" }}>
                        <DataOriginBadge origin={dataOrigin} />
                        <span style={{ fontFamily: "var(--mono)", fontSize: 9, opacity: 0.35 }}>
                        ...
                        {[100, 85, 65].map((w, i) => (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: 4,
                              width: `${w}%`,
                              animation: `dot-pulse 1.5s ${i * 0.1}s infinite`,
                            }}
                          />
                        ))}
                      </div>
                ...
                {activeTab === "lead" && (
                  <div style={{ maxWidth: 860, margin: "0 auto" }}>
                    ...
                    {!leads.length ? (
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {leads.map((l: Lead, i: number) => (
                          <div
                            key={l.id ?? i}
                            className="ea-glass ea-fadeup"
                            style={{ padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", animationDelay: `${i * 0.06}s` }}
                            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,200,83,0.3)"; }}
                            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,255,163,0.10)"; }}
                          >
                            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                              <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--ok)", boxShadow: "0 0 10px var(--ok)" }} />
                              <div>
                                <p style={{ fontFamily: "var(--mono)", fontSize: 8.5, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--ok)", marginBottom: 3 }}>
                                  {l.audit_verdict}
                                </p>
                                <p style={{ fontFamily: "var(--mono)", fontSize: 11, fontWeight: 700 }}>
                                  {l.location_data} — <span style={{ opacity: 0.4 }}>{l.target_value}</span>
                                </p>
                              </div>
                            </div>
                            <button className="ea-btn-ghost" onClick={() => generatePDF(l.output_text ?? "")}>
                              Extract
                            </button>
                          </div>
                        ))}
                ...
                {activeTab === "contract" && (
                  <div style={{ maxWidth: 860, margin: "0 auto" }}>
                    ...
                    <div className="ea-glass" style={{ padding: "20px", borderColor: "rgba(0,255,163,0.2)", background: "rgba(0,255,163,0.04)" }}>
                      ...
                      <p style={{ fontFamily: "var(--serif)", fontSize: 12, opacity: 0.5, lineHeight: 1.6 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                        {history.map((h: HistoryItem, i: number) => (
                          <div
                            key={h.id ?? i}
                            className="ea-glass ea-fadeup"
                            style={{ padding: "18px", position: "relative", animationDelay: `${i * 0.05}s` }}
                          >
                            <div style={{ position: "absolute", top: 14, right: 14, display: "flex", gap: 12, alignItems: "center" }}>
                              <span style={{ fontFamily: "var(--mono)", fontSize: 8.5, opacity: 0.3 }}>
                  ...
                  <div style={{ maxWidth: 860, margin: "0 auto" }}>
                    ...
                    {!darkPoolDeals.length ? (
                      <div className="ea-glass" style={{ padding: 40, textAlign: "center" }}>
                        ...
                        <p className="ea-label" style={{ fontSize: 8, opacity: 0.5 }}>
                          Enter location to scan developer CRMs, court auctions, and broker networks.
                        </p>
                      </div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {darkPoolDeals.map((deal: DarkPoolDeal, i: number) => (
                          <div
                            key={deal.id}
                            className="ea-glass ea-dp-card ea-fadeup"
                            style={{ border: "1px solid rgba(255,77,46,0.2)", background: "rgba(255,77,46,0.04)", animationDelay: `${i * 0.07}s`, position: "relative" }}
                          >
                              ...
                              <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                                    ...
                                    <span style={{ fontFamily: "var(--mono)", fontSize: 8, opacity: 0.3, textTransform: "uppercase" }}>{deal.source}</span>
                                  </div>
                                  <p style={{ fontFamily: "var(--mono)", fontSize: 11, fontWeight: 700, marginBottom: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                    {deal.location} — <span style={{ opacity: 0.4 }}>{deal.propertyType}</span>
                                  </p>
                                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                                    ...
                                    {deal.minInvestment && (
                                      <span style={{ fontFamily: "var(--mono)", fontSize: 8.5, opacity: 0.5 }}>Min: {deal.minInvestment}</span>
                                    )}
                                  </div>
                                  {deal.reason && (
                                    <p style={{ fontFamily: "var(--mono)", fontSize: 8, color: "#FF4D2E", opacity: 0.8 }}>{deal.reason}</p>
                                  )}
                                  {deal.expiresAt && (
                    ...
                    {!portfolioAnalysis ? (
                      <div className="ea-glass" style={{ padding: 28 }}>
                        ...
                        <textarea
                          ...
                          rows={5}
                          style={{ ...inputStyle, height: "auto", resize: "vertical", marginBottom: 12, color: "var(--text)", background: "rgba(0,0,0,0.5)", fontSize: 10, lineHeight: 1.5 }}
                        />
                        <button className="ea-btn-primary" onClick={handlePortfolioUpload}>
                          ANALYZE PORTFOLIO
                        </button>
                      </div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        <div className="ea-glass" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", overflow: "hidden" }}>
                          {[
                            ...
                            <div key={i} style={{ padding: "14px 16px", borderRight: i < 3 ? "1px solid rgba(0,255,163,0.1)" : "none" }}>
                              <div className="ea-label" style={{ fontSize: 7.5, marginBottom: 6 }}>{metric.l}</div>
                              <div style={{ fontFamily: "var(--mono)", fontSize: 20, fontWeight: 700, color: metric.c }}>{metric.v}</div>
                            </div>
                          ))}
                        </div>
                        ...
                        {portfolioAnalysis.rebalanceSignals.length > 0 && (
                          <div className="ea-glass" style={{ borderColor: "rgba(255,77,46,0.2)", background: "rgba(255,77,46,0.04)", padding: "14px 16px" }}>
                            ...
                            {portfolioAnalysis.rebalanceSignals.map((sig: RebalanceSignal, i: number) => (
                              <div key={i} className="ea-glass" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", marginBottom: 6, background: "rgba(0,0,0,0.3)" }}>
                                <div style={{
                                  ...
                                  background: sig.action === "SELL" ? "#FF4D2E" : sig.action === "REDUCE" ? "#FF6B00" : "#FFB800",
                                }} />
                                <div style={{ flex: 1 }}>
                                  <div style={{ display: "flex", gap: 6, marginBottom: 2 }}>
                                    <span style={{ fontFamily: "var(--mono)", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>{sig.action}</span>
                                    <span style={{ fontFamily: "var(--mono)", fontSize: 8, opacity: 0.35 }}>{sig.assetId}</span>
                                  </div>
                                  <p style={{ fontFamily: "var(--mono)", fontSize: 8, opacity: 0.5 }}>{sig.reason}</p>
                                </div>
                                <span style={{
                                  padding: "3px 8px", borderRadius: 4,
                                  fontFamily: "var(--mono)", fontSize: 7.5, fontWeight: 700, color: "var(--bg)",
                                  background: sig.urgency === "IMMEDIATE" ? "#FF4D2E" : sig.urgency === "30_DAYS" ? "#FF6B00" : "#FFB800",
                                }}>
                                  {sig.urgency}
                                </span>
                              </div>
                  ...
                  <div style={{ maxWidth: 860, margin: "0 auto", display: "flex", gap: 10, alignItems: "center" }}>
                    <div style={{ flex: 1, position: "relative" }}>
                      <input
                        ...
                        onKeyDown={(e) => { if (e.key === "Enter") handleAction(undefined, false, e, "CHAT_COMMAND"); }}
